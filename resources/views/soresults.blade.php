    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-4">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-8">
                <h1 class="text-2xl font-bold text-gray-800 mb-2">Results for SO Number: {{ $docNum }}</h1>
                <h2 class="text-xl font-semibold text-gray-600 mb-2">Customer: {{ $customer }}</h2>
                <h2 class="text-xl font-semibold text-gray-600 mb-6">Date: {{ $date }}</h2>
                ...
                @if (session('success'))
                    <div
                        ...
                        {{ session('success') }}
                        <button type="button" class="text-green-800 hover:text-green-900 ml-2 text-2xl"
                ...
                @if ($errors->any())
                    <div
                        ...
                        <button type="button" class="text-red-800 hover:text-red-900 ml-2 text-2xl"
                            onclick="this.parentElement.style.display='none';">&times;</button>
                    </div>
                @endif
                ...
                @if ($data->isEmpty())
                    <p class="text-red-500 text-lg">No data found for this SO Number.</p>
                @else
                    <table class="min-w-full bg-white border-collapse border border-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="border border-gray-300 px-4 py-2 text-left">Delivery Quantity</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($data as $item)
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="border border-gray-300 px-4 py-2">{{ $item->scannedCount }} /
                                        {{ number_format($ctn) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot class="bg-gray-100">
                            <tr>
                                <th colspan="3" class="border border-gray-300 px-4 py-2 text-left">Total</th>
                                <th class="border border-gray-300 px-4 py-2">{{ $totalQuantity }}</th>
                                <th class="border border-gray-300 px-4 py-2">{{ number_format($totalCTN) }}</th>
                ...
                @if ($allFinished)
                    <a href="{{ route('update.so.data', ['docNum' => $docNum]) }}"
                ...
                @else
                    <p class="text-red-500 mt-6">Not all items are finished yet.</p>
                @endif
                ...
                <h2 class="text-xl font-bold text-gray-800 mt-10">Scanned Data</h2>

                @forelse ($scandatas as $itemCode => $scans)
                    <h3 class="text-lg font-semibold text-gray-700 mt-4">Item Code: {{ $itemCode }}</h3>
                    <table class="min-w-full bg-white border-collapse border border-gray-200 mt-2">
                        <thead class="bg-gray-100">
                            <tr>
                                <th class="border border-gray-300 px-4 py-2 text-left">No</th>
                                <th class="border border-gray-300 px-4 py-2 text-left">Quantity</th>
                                <th class="border border-gray-300 px-4 py-2 text-left">Warehouse</th>
                        ...
                        <tbody>
                            @foreach ($scans as $scan)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="border border-gray-300 px-4 py-2">{{ $scan->quantity }}</td>
                                    <td class="border border-gray-300 px-4 py-2">{{ $scan->warehouse }}</td>
                                    <td class="border border-gray-300 px-4 py-2">{{ $scan->label }}</td>
                                    <td class="border border-gray-300 px-4 py-2">{{ $scan->created_at }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                @empty
                    <p class="text-red-500 text-lg">No scanned data yet for this SO Number.</p>
                @endforelse
            </div>
        </div>
    </div>
