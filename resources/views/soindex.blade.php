<x-app-layout>
    @if (session('success'))
        <div class="flex items-center p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg" role="alert">
            <svg class="w-5 h-5 mr-2 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="flex justify-center mb-6">
                    <form action="{{ route('so.filter') }}" method="GET">
                        ...
                        <select name="is_done" id="is_done" class="border rounded p-2" onchange="this.form.submit()">
                            <option value="all" {{ request('is_done') === 'all' ? 'selected' : '' }}>
                                All
                            </option>
                            <option value="1" {{ request('is_done') === '1' ? 'selected' : '' }}>
                                Done
                            </option>
                            <option value="0" {{ request('is_done') === '0' ? 'selected' : '' }}>
                                Not Done
                            </option>
                        </select>
                    </form>
                ...
                <h1 class="text-2xl font-semibold text-gray-800 mb-6 text-center mt-5">
