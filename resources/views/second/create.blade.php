<x-app-layout>
    <div class="max-w-3xl mx-auto py-8">
        <h1 class="text-2xl font-bold mb-6 text-center">Create Planning Entry</h1>

        <form action="{{ route('second.daily.process.store') }}" method="POST" x-data="{ repeatableSections: [{}] }" class="space-y-6">
            @csrf
            ...
            <template x-for="(section, index) in repeatableSections" :key="index">
                <div class="space-y-4 bg-gray-50 p-6 shadow-md rounded-md">
                    <div>
                        <label :for="'line_' + index" class="block text-sm font-medium text-gray-700">Line</label>
                    ...
                    <div>
                        <label :for="'item_code_' + index" class="block text-sm font-medium text-gray-700">Item
                            Code</label>
                        <select :name="'item_code[]'"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                            x-data="{}" x-init="$nextTick(() => new TomSelect($el, {
                                valueField: 'item_code',
                                labelField: 'item_code',
                                searchField: 'item_code',
                                load: function(query, callback) {
                                    if (!query.length) return callback();
                                    fetch('{{ route('api.items') }}?query=' + encodeURIComponent(query))
                                        .then(response => response.json())
                                        .then(data => { callback(data); })
                                        .catch(() => { callback(); });
                                }
                            }))" x-model="section.itemCode" required>
                            <option value="" selected disabled>Search Item Code</option>
                        </select>
                    </div>

                    <div>
                        <label :for="'item_description_' + index" class="block text-sm font-medium text-gray-700">Item
                            Description</label>
                    ...
                    <div>
                        <label :for="'quantity_hour_' + index" class="block text-sm font-medium text-gray-700">Quantity
                            Hour</label>
                    ...
                    <div>
                        <label :for="'process_time_' + index" class="block text-sm font-medium text-gray-700">Process
                            Time</label>
                    ...
                    <div>
                        <label :for="'quantity_plan_' + index" class="block text-sm font-medium text-gray-700">Quantity
                            Plan</label>
